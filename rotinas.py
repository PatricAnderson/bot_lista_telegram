...
async def disparar_troca_por_categoria(client_bot=None):
    logger.info("🔄 Executando rotina: disparar_troca_por_categoria (12h/20h)...")[cite: 9]
    
    # Em caso de falha na passagem, utiliza o bot importado do config
    client_bot = client_bot or bot

    if not client_bot:
        logger.error("❌ ERRO CRÍTICO: O client não está disponível para o disparo!")[cite: 9]
        return
    ...
    try:
        async with db_pool.acquire() as conn:
            ...
            if not canais or not canais_destino:
                logger.warning(f"⚠️ Canais insuficientes no banco.")[cite: 9]
                return

        canais_por_categoria = {cat: [] for cat in CATEGORIAS_DISPONIVEIS.keys()}[cite: 9]
        for c in canais:
        ...
        for destino in canais_destino:
            ...
            if ultima_msg_id:
                try:
                    ...
                    logger.info(f"🗑️ Lista anterior (ID: {ultima_msg_id}) apagada no canal {chat_id_destino}")[cite: 9]
            ...
            reais = [c for c in pool_canais if not c['semente']][cite: 9]
            sementes = [c for c in pool_canais if c['semente']][cite: 9]

            random.shuffle(reais)[cite: 9]
            random.shuffle(sementes)[cite: 9]
        ...
        logger.info("✅ Rotina finalizada com sucesso.")[cite: 9]
    except Exception as e:
        logger.error(f"❌ Erro crítico: {e}")

async def monitorar_membros_semanal():
    logger.info("👥 Executando rotina: monitorar_membros_semanal...")[cite: 9]
    try:
        async with db_pool.acquire() as conn:
            pass
        logger.info("✅ Rotina 'monitorar_membros_semanal' finalizada com sucesso.")[cite: 9]
    except Exception as e:
        logger.error(f"❌ Erro na rotina: {e}")

def iniciar_agendamentos(client_bot=None):
    logger.info("⏰ Registrando rotinas no agendador...")[cite: 9]
    ...
    scheduler.start()
    logger.info("✅ Todas as rotinas registradas.")[cite: 9]
