...
async def start_command(message):
    ...
    try:
        ...
        if ADMIN_ID and user_id == ADMIN_ID:
            # Insere no topo
            markup.keyboard.insert(0, [InlineKeyboardButton("🛠️ Painel Admin", callback_data="admin_painel")])
...
async def importar_fakes(message):
    ...
    if not message.reply_to_message or not message.reply_to_message.document:
        await bot.reply_to(message, "⚠️ Responda a um arquivo `.txt` com: `/importar <categoria>`", parse_mode="Markdown")
        return
        
    partes = message.text.split()
    if len(partes) < 2 or partes[1] not in CATEGORIAS_DISPONIVEIS:
        await bot.reply_to(message, "⚠️ **Categoria inválida!**", parse_mode="Markdown")
        return
        
    categoria_alvo = partes[1]
    msg_status = await bot.reply_to(message, "⏳ Processando...")
    ...
    try:
        ...
        await bot.edit_message_text(f"🎉 **{adicionados}** canais sementes injetados em `{categoria_alvo}`.", message.chat.id, msg_status.message_id, parse_mode="Markdown")
    except Exception as e:
        await bot.edit_message_text(f"❌ **Erro:** {e}", message.chat.id, msg_status.message_id, parse_mode="Markdown")
...
async def admin_command(message):
    ...
    await bot.reply_to(message, "🛠️ **Painel de Administração**", reply_markup=markup, parse_mode="Markdown")

@bot.message_handler(commands=['testar'])
async def testar_comando(message):
    try:
        await bot.reply_to(message, "🔄 Forçando disparo da rotina de troca...")
        await disparar_troca_por_categoria(bot)
        await bot.reply_to(message, "✅ Rotina de teste finalizada. Verifique os logs!")
    except Exception as e:
        await bot.reply_to(message, f"❌ Falha ao disparar o teste: {e}")
...
async def capturar_texto_admin(message):
    ...
    try:
        ...
        if estado["etapa"] == "aguardando_titulo":
            ...
            await bot.reply_to(message, "🎉 **Link Fixo cadastrado!**", reply_markup=markup, parse_mode="Markdown")
    except Exception as e:
